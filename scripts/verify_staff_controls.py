...
def run(name, command):
    ...
    clean=lambda value:value.replace(str(ROOT),"<repo>")
...
def main():
    ...
    with tempfile.TemporaryDirectory() as temporary:
        ...
        compose=run("docker_compose_config",["docker","compose","-f",str(target/"compose.yaml"),"config"])
        compose["command"]=compose["command"].replace(str(target.parent),"<temp>"); compose["stdout"]=compose["stdout"].replace(str(target.parent),"<temp>"); compose["stderr"]=compose["stderr"].replace(str(target.parent),"<temp>")
        scenarios.append({"name":"kubernetes_policy_validation","status":"passed" if validation["valid"] else "failed","evidence":validation}); scenarios.append(compose)
